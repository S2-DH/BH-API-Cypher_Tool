...
function Format-APIResult {
    ...
    # ── API RESULTS (standard JSON responses) ──
        ...
        if ($data.PSObject -and $data.PSObject.Properties) {
            ...
            if ($propNames -contains 'data') {
                ...
                elseif ($inner -is [PSCustomObject] -or $inner -is [System.Management.Automation.PSCustomObject]) {
                    # Single object wrapped in data envelope - e.g. { data: { members: [...] } }
                    $rawData = $inner

                    # Auto-unwrap: if this object has exactly one array-valued property
                    # (members, nodes, items, results, etc.), treat that as the item list
                    # instead of dumping it as raw JSON text.
                    $innerArrayProps = @($inner.PSObject.Properties | Where-Object { $_.Value -is [System.Array] })
                    if ($innerArrayProps.Count -eq 1) {
                        $items = $innerArrayProps[0].Value
        ...
        # Top-level object with a single array property and no 'data' envelope at all
        # e.g. { members: [...] } returned directly
        if (-not $items -and $data.PSObject -and $data.PSObject.Properties) {
            $topArrayProps = @($data.PSObject.Properties | Where-Object { $_.Value -is [System.Array] })
            if ($topArrayProps.Count -eq 1) {
                $items = $topArrayProps[0].Value
                $totalCount = $items.Count
            }
        }
        ...
        if ($items -and $items.Count -gt 0) {
            ...
            if ($isPrimitive) {
                # Flat string/number array (e.g. /api/v2/attack-path-types) - show all
                $counter = 0
                $displayObjects = @()
                ...
                # Table display - console preview capped at 100 rows for readability,
                # but $displayObjects (used for export below) always contains the FULL set
                if ($displayObjects.Count -gt 100) {
                    $displayObjects[0..99] | Format-Table -AutoSize -Wrap | Out-String | Write-Host
                    Write-Host "  ... $($displayObjects.Count) total, showing first 100 in console (full set included in export)" -ForegroundColor DarkGray
                } else {
...
function Get-APILibrary {
    ...
    return @(
        ...
        @{ Category = "Attack Paths"; Name = "Domain Available Paths"; Description = "Available paths for domain - requires asset_group_tag_id (1=TierZero)"; Endpoint = "/api/v2/domains/{domain_id}/available-attack-paths?asset_group_tag_id={asset_group_tag_id}" },
        @{ Category = "Attack Paths"; Name = "Domain Path Details"; Description = "Path details for domain - requires asset_group_tag_id (1=TierZero)"; Endpoint = "/api/v2/domains/{domain_id}/attack-path-findings?asset_group_tag_id={asset_group_tag_id}" },
        ...
        @{ Category = "Findings"; Name = "Hygiene Finding Catalog (All Possible vs Active)"; Description = "** Fetches full catalog from attack-path-types + compares to active findings: shows every possible finding type marked as ACTIVE or not yet triggered."; Endpoint = "SPECIAL:HygieneCatalog" },
        @{ Category = "Findings"; Name = "Full Attack Path Export (All Types, All Rows)"; Description = "** Loops every active finding type for a tag and pulls FULL row-level detail (not just counts) into one combined export - e.g. all 3200+ individual Attack Path rows in one CSV."; Endpoint = "SPECIAL:FullAttackPathExport" }
    )
}
...
function Invoke-FindingDetails {
    ...
    Write-Host "  [>] GET /api/v2/domains/$domainId/available-types?asset_group_tag_id=$tagId" -ForegroundColor Yellow
...
function Invoke-ActiveFindingTypes {
    ...
    foreach ($ft in $findingTypes) {
        ...
        $detailsEndpoint = '/api/v2/domains/' + $domainId + '/details?finding=' + $ft + '&skip=0&limit=1000&asset_group_tag_id=' + $tagId + '&Accepted=eq%3Afalse'
        Write-Host "  [>] GET $detailsEndpoint" -ForegroundColor DarkGray
...
function Invoke-FindingSummary {
    ...
    foreach ($findingType in $findingTypes) {
        ...
        $summaryDetailsEndpoint = '/api/v2/domains/' + $domainId + '/details?finding=' + $findingType + '&skip=0&limit=1000&sort_by=-exposure_percentage&sort_by=-exposure_count&asset_group_tag_id=' + $tagId + '&Accepted=eq%3Afalse'
        Write-Host "  [>] GET $summaryDetailsEndpoint" -ForegroundColor DarkGray

        $detailsResult = Invoke-BHERequest -BaseUrl $BaseUrl `
            -Endpoint $summaryDetailsEndpoint `
            -TokenId $TokenId -TokenKey $TokenKey -Silent
...
function Invoke-HygieneCatalog {
    param(
        [string]$BaseUrl,
        [string]$TokenId,
        [string]$TokenKey
    ...
    Write-Host ""
    Write-Host "  -- Hygiene Finding Catalog ------------------------------------" -ForegroundColor DarkCyan
    Write-Host "  Shows ALL possible finding types BHE supports, marked as" -ForegroundColor DarkGray
    Write-Host "  Active (found in your domain) or Not Yet Triggered." -ForegroundColor DarkGray
    Write-Host ""

    $domainId = (Read-Host "  domain_id").Trim()
    if ([string]::IsNullOrWhiteSpace($domainId)) { return }

    $tagIdInput = (Read-Host "  asset_group_tag_id  1=TierZero 3=TierOne  blank=1").Trim()
    if ($tagIdInput) { $tagId = $tagIdInput } else { $tagId = "1" }

    Write-Host ""
    Write-Host "  [1/2] Fetching master catalog (all possible finding types)..." -ForegroundColor DarkGray

    # attack-path-types returns the full list of every possible finding type string
    Write-Host "  [>] GET /api/v2/attack-path-types" -ForegroundColor Yellow
    $catalogResult = Invoke-BHERequest -BaseUrl $BaseUrl `
        -Endpoint "/api/v2/attack-path-types" `
        -TokenId $TokenId -TokenKey $TokenKey -Silent

    $allTypes = @()
    if ($catalogResult.Success) {
        $d = $catalogResult.Data
        # Response may be: { data: [...] } or { finding_types: [...] } or a bare array
        if     ($d.data           -is [System.Array]) { $raw = $d.data }
        elseif ($d.finding_types  -is [System.Array]) { $raw = $d.finding_types }
        elseif ($d.types          -is [System.Array]) { $raw = $d.types }
        elseif ($d                -is [System.Array]) { $raw = $d }
        else {
            # Dump all property names to help diagnose
            $props = ($d | Get-Member -MemberType NoteProperty | Select-Object -ExpandProperty Name) -join ', '
            Write-Host "  [!] Unexpected catalog response shape. Properties: $props" -ForegroundColor Yellow
            $raw = @()
        }
        foreach ($item in $raw) {
            if ($item -is [string]) {
        ...
        Write-Host "  [!] Failed to retrieve finding catalog." -ForegroundColor Red
        return
    }

    Write-Host "  [*] $($allTypes.Count) finding types in catalog." -ForegroundColor DarkGray
    Write-Host ""
    Write-Host "  [2/2] Fetching active finding types for domain..." -ForegroundColor DarkGray

    # Use the exact same endpoint as entry 161 (Invoke-ActiveFindingTypes) - proven to work
    Write-Host "  [>] GET /api/v2/domains/$domainId/available-types?asset_group_tag_id=$tagId" -ForegroundColor Yellow
    $typesResult = Invoke-BHERequest -BaseUrl $BaseUrl `
        -Endpoint "/api/v2/domains/$domainId/available-types?asset_group_tag_id=$tagId" `
        -TokenId $TokenId -TokenKey $TokenKey -Silent
    ...
    if ($typesResult.Success) {
        $td = $typesResult.Data
        if     ($td.data -is [System.Array]) { $activeList = $td.data }
        elseif ($td      -is [System.Array]) { $activeList = $td }
        else                                 { $activeList = @() }

        foreach ($ft in $activeList) {
            if ($ft -is [string]) { $activeSet[$ft] = $true }
        }
    }

    Write-Host "  [*] $($activeSet.Count) active finding types detected." -ForegroundColor DarkGray
    Write-Host ""

    # Build comparison rows
    $rows = @()
    foreach ($typeId in ($allTypes | Sort-Object)) {
        if ($activeSet.ContainsKey($typeId)) { $status = "ACTIVE" } else { $status = "not triggered" }
        $rows += [PSCustomObject]@{
            Status = $status
            TypeID = $typeId
        }
    ...
    $inactiveRows = @($rows | Where-Object { $_.Status -ne "ACTIVE" })

    Write-Host "  =================================================" -ForegroundColor DarkCyan
    Write-Host "   HYGIENE FINDING CATALOG" -ForegroundColor Cyan
    Write-Host "   Domain : $domainId" -ForegroundColor DarkGray
    Write-Host "   Tag ID : $tagId" -ForegroundColor DarkGray
    Write-Host "   $($activeRows.Count) active  |  $($inactiveRows.Count) not triggered  |  $($rows.Count) total possible" -ForegroundColor White
    Write-Host "  =================================================" -ForegroundColor DarkCyan
    Write-Host ""

    Write-Host "  [ACTIVE - findings detected in your environment]" -ForegroundColor Green
    Write-Host ""
    foreach ($r in ($activeRows | Sort-Object TypeID)) {
        Write-Host ("  >> ACTIVE   {0}" -f $r.TypeID) -ForegroundColor Green
    }
    if ($activeRows.Count -eq 0) {
        Write-Host "  (none - check domain_id and tag_id)" -ForegroundColor DarkYellow
    }

    Write-Host ""
    Write-Host "  [NOT YET TRIGGERED - possible but no findings detected]" -ForegroundColor DarkYellow
    Write-Host ""
    foreach ($r in ($inactiveRows | Sort-Object TypeID)) {
        Write-Host ("     not triggered   {0}" -f $r.TypeID) -ForegroundColor DarkGray
    }
    ...
    $exportChoice = Read-Host "  Export to CSV? File path or blank to skip"
    if (-not [string]::IsNullOrWhiteSpace($exportChoice)) {
        Export-Results -Objects $rows -Path $exportChoice
    }
...
function Invoke-FullAttackPathExport {
    param(
        [string]$BaseUrl,
        [string]$TokenId,
        [string]$TokenKey
    ...
    Write-Host ""
    Write-Host "  -- Full Attack Path Export (All Types, All Rows) ---------------" -ForegroundColor DarkCyan
    Write-Host "  Loops every active finding type and pulls full row-level detail" -ForegroundColor DarkGray
    Write-Host "  into ONE combined export - not just grouped counts." -ForegroundColor DarkGray
    Write-Host ""
    ...
    $perTypeLimitStr = (Read-Host "  Max rows per finding type  blank=1000").Trim()
    if ([string]::IsNullOrWhiteSpace($perTypeLimitStr)) { $perTypeLimit = 1000 } else { $perTypeLimit = [int]$perTypeLimitStr }

    $includeAccepted = (Read-Host "  Include risk-accepted findings too?  y/N").Trim()
    $acceptedFilter = if ($includeAccepted -eq 'y' -or $includeAccepted -eq 'Y') { "" } else { "&Accepted=eq%3Afalse" }
    if ($acceptedFilter -eq "") {
        Write-Host "  [*] Including ALL findings (accepted + unaccepted)" -ForegroundColor DarkGray
    } else {
        Write-Host "  [*] Excluding risk-accepted findings (unaccepted only)" -ForegroundColor DarkGray
    }
    ...
    Write-Host "  [+] Found $($findingTypes.Count) active finding types" -ForegroundColor Green
    Write-Host ""
    Write-Host "  [2/2] Fetching full row-level detail for each type..." -ForegroundColor White
    Write-Host ""

    # Per BHE dashboard tooltip: "Tier Zero findings contribute Attack Paths based on
    # measured Exposure, while Kerberoastable or Large Default Group findings contribute
    # based on measured Impact." Everything else (T0*, ADCS ESC*, Azure T0*) uses Exposure.
    function Get-AttackPathMetricType($findingTypeName) {
        if ($findingTypeName -like 'LargeDefaultGroups*') { return 'Impact' }
        if ($findingTypeName -eq 'Kerberoasting')          { return 'Impact' }
        if ($findingTypeName -eq 'ASREPRoasting')           { return 'Impact' }
        return 'Exposure'
    }

    # Helper to shorten SIDs/GUIDs to last segment for display
    function Shorten-ExportID($id) {
        if ([string]::IsNullOrWhiteSpace($id)) { return "" }
        return $id
    }
    ...
    foreach ($findingType in $findingTypes) {
        ...
        $exportDetailsEndpoint = '/api/v2/domains/' + $domainId + '/details?finding=' + $findingType + '&skip=0&limit=' + $perTypeLimit + '&sort_by=-exposure_percentage&sort_by=-exposure_count&asset_group_tag_id=' + $tagId + $acceptedFilter
        Write-Host "  [>] GET $exportDetailsEndpoint" -ForegroundColor DarkGray

        $detailsResult = Invoke-BHERequest -BaseUrl $BaseUrl `
            -Endpoint $exportDetailsEndpoint `
            -TokenId $TokenId -TokenKey $TokenKey -Silent
        ...
        if ($detailsResult.Success) {
            ...
            if ($items -and $items.Count -gt 0) {
                $rowsThisType = $items.Count
                foreach ($item in $items) {
                    $fromShort = Shorten-ExportID $item.FromPrincipal
                    $toShort   = Shorten-ExportID $item.ToPrincipal
                    $fromKind  = if ($item.FromPrincipalKind) { $item.FromPrincipalKind } else { "" }
                    $toKind    = if ($item.ToPrincipalKind)   { $item.ToPrincipalKind }   else { "" }
                    $severity  = if ($item.Severity)          { $item.Severity }          else { "" }
                    $exposure  = if ($item.ExposureCount)     { $item.ExposureCount }     elseif ($item.exposure_count) { $item.exposure_count } else { "" }
                    ...
                    $createdAt = ""
                    if ($item.created_at) {
                        try { $createdAt = $item.created_at.ToString().Substring(0,10) } catch { $createdAt = "$($item.created_at)" }
                    }

                    $metricType = Get-AttackPathMetricType $findingType
                    $metricValue = if ($metricType -eq 'Impact') { $impact } else { $exposure }
                    if ($metricValue -and $metricValue -ne "") {
                        $computedTotal += [int]$metricValue
                    }
        ...
        Write-Host "  -> $rowsThisType rows" -ForegroundColor DarkGreen
        $typeSummary += [PSCustomObject]@{ FindingType = $findingType; Rows = $rowsThisType }
    }

    Write-Host ""
    Write-Host "  =================================================" -ForegroundColor DarkCyan
    Write-Host "   FULL ATTACK PATH EXPORT" -ForegroundColor Cyan
    Write-Host "   Domain : $domainId" -ForegroundColor DarkGray
    Write-Host "   Tag ID : $tagId" -ForegroundColor DarkGray
    Write-Host "   $($findingTypes.Count) finding types  |  $($allRows.Count) total rows collected" -ForegroundColor White
    Write-Host "   Computed Attack Paths Total (sum of Exposure/Impact): $computedTotal" -ForegroundColor Magenta
    Write-Host "   Compare this to the dashboard 'Attack Paths' tile - should be close/equal" -ForegroundColor DarkGray
    Write-Host "  =================================================" -ForegroundColor DarkCyan
    Write-Host ""

    $capHit = @($typeSummary | Where-Object { $_.Rows -eq $perTypeLimit })
    if ($capHit.Count -gt 0) {
        Write-Host "  [!] Warning: these finding types returned exactly $perTypeLimit rows -" -ForegroundColor Yellow
        Write-Host "      they may have MORE rows than the per-type limit allowed. Re-run with" -ForegroundColor Yellow
        Write-Host "      a higher 'Max rows per finding type' value if you need every single row:" -ForegroundColor Yellow
        foreach ($c in $capHit) {
            Write-Host "        - $($c.FindingType)" -ForegroundColor Yellow
        }
        Write-Host ""
    }

    if ($allRows.Count -gt 0) {
        $allRows | Select-Object -First 30 | Format-Table -AutoSize | Out-String | Write-Host
        if ($allRows.Count -gt 30) {
            Write-Host "  ... $($allRows.Count) total rows, showing first 30 in console (full set included in export)" -ForegroundColor DarkGray
        }
    } else {
        Write-Host "  [*] No rows returned across any finding type." -ForegroundColor DarkYellow
    }

    Write-Host ""
    $exportChoice = Read-Host "  Export ALL $($allRows.Count) rows to CSV? File path or blank to skip"
    if (-not [string]::IsNullOrWhiteSpace($exportChoice)) {
        Export-Results -Objects $allRows -Path $exportChoice
    }
...
function Show-APILibrary {
    ...
    while ($true) {
        ...
        if ([int]::TryParse($pick, [ref]$pickNum) -and $queryMap.ContainsKey($pickNum)) {
            ...
            # Special handler - Hygiene Finding Catalog
            if ($endpoint -eq 'SPECIAL:HygieneCatalog') {
                Invoke-HygieneCatalog -BaseUrl $BaseUrl -TokenId $TokenId -TokenKey $TokenKey
                continue
            }

            # Special handler - Full Attack Path Export (all types, all rows)
            if ($endpoint -eq 'SPECIAL:FullAttackPathExport') {
                Invoke-FullAttackPathExport -BaseUrl $BaseUrl -TokenId $TokenId -TokenKey $TokenKey
                continue
            }
